...
import { FaFire, FaStar, FaTrophy, FaSnowflake, FaCheckCircle } from "react-icons/fa";
...
export default function LearningHabitPage() {
    ...
    // Function to open the progress modal for daily and weekly habits
    function openProgressModal(h: LearningHabit) {
        setHabitToMark(h);
        // For daily habits, set the value to today's progress or 1 if none exists
        if (h.frequency === 'daily') {
            ...
            // For weekly habits, set the value to the current week's progress
            const today = new Date();
    ...
    async function submitProgress() {
        ...
        try {
            ...
            // Set the progress count based on frequency
            let count = progressValue;
            if (habitToMark.frequency === 'weekly') {
                count = progressValue; // The input field will capture the count for weekly
            }

            const res = await axios.put(`/api/learninghabit/${habitToMark._id}`, { progress: { date: today.toISOString(), count: count } });
    ...
    // Checks if two dates are in the same ISO week (Mon-Sun).
    ...
    // Function to check if a weekly habit's target is met for the current week.
                ...
                <>
                    ...
                    <div className="max-w-7xl w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                        {habits.map((h) => {
                            const today = new Date();
                            today.setHours(0, 0, 0, 0);
                            const doneToday = h.frequency === 'daily' && h.progress.some((p) => isSameDay(new Date(p.date), today));
                            const isWeeklyCompleted = h.frequency === 'weekly' && isWeeklyTargetMet(h);
                            const currentMonthInfo = dateInfo(h._id);
                            const calendarDays = getCalendarDays(currentMonthInfo.year, currentMonthInfo.month);

                            // Calculate freezes for the current month on the frontend
                                ...
                                >
                                        ...
                                        {h.frequency === 'daily' ? (
                                            <>
                                                <button
                                                    ...
                                                    className={`px-2 py-1 rounded flex items-center justify-center text-white ${doneToday
                                                ...
                                                <button
                                                    disabled={doneToday || currentMonthFreezes >= 2}
                                                    title={
                                            ...
                                            <button
                                                disabled={isWeeklyTargetMet(h)}
                                                onClick={(e) => {
                                                    e.stopPropagation();
                                                    openProgressModal(h);
                                                }}
                                                className={`col-span-2 px-2 py-1 rounded flex items-center justify-center text-white ${isWeeklyTargetMet(h)
                                                    ? "bg-green-700 cursor-not-allowed"
                                                    : "bg-green-600 hover:bg-green-700 cursor-pointer"
                                                    }`}
                                            >
                                                {isWeeklyTargetMet(h) ? "Done This Week" : "Mark This Week"}
                                            </button>
                                        )}
                                    ...
                                    <div className="grid grid-cols-7 gap-1 select-none">
                                        {calendarDays.map((day, idx) => {
                                            ...
                                            // Determine if a weekly habit's day is part of a completed week
                                            const isWeeklyCompletedDay = h.frequency === 'weekly' && h.progress.some(p => isSameWeek(new Date(p.date), day));
